...
void MetricP2L2UCond(const double deriv[4], double metric[3], double maxCond=-1.){
	...
	if(ThirdDegreeReal(deriv[1], 2*deriv[2]-deriv[0], deriv[3]-2*deriv[1], -deriv[2], nearest) >0){// si dans ce cas, trois points minima locaux, parmi lesquels if faut choisir.
		double dist[3];
