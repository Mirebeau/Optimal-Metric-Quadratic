...
void Rotation(double theta, double R[2][2]);
void BiRotation(double theta1, double theta2, double R[2][2]);
...
void SquareDiagToSym2(const double A[2][2], const double D[2], double metric[3]){
	metric[0] = D[0]*square(A[0][0])  + D[1]*square(A[1][0]);
	metric[1] = D[0]*A[0][0]* A[0][1] + D[1]*A[1][0]*A[1][1];
	metric[2] = D[0]*square(A[0][1])  + D[1]*square(A[1][1]);
};
...
void BiRotation(double t1, double t2, double R[2][2]){
	double alpha = 1/sin(t1-t2);
	R[0][0] = -sin(t2)*alpha;
	R[0][1] = cos(t2)*alpha;
	R[1][0] = sin(t1)*alpha;
	R[1][1] = -cos(t1)*alpha;
